...
/*****  HOW THE MAP WORKS  ************************************************
  •   world_map.svg lives in assets/ and must have ISO‑2 ids ("us", "fr", …).
...
function updateCounter() {
  ...
  if (el) el.textContent = `${seenFlags.size} / ${countries.length} seen`;
}
...
function highlightMap() {
  ...
  if (!svgRoot || !svgRoot.getElementById) return; // not ready yet

  ensureHighlightStyle(svgRoot);

  // Remove previous highlights
  svgRoot.querySelectorAll(".country-seen").forEach(el => el.classList.remove("country-seen"));

  // Add highlights
  seenFlags.forEach(code => {
    const upper = code.toUpperCase();
    const lower = code.toLowerCase();
    let target = svgRoot.getElementById(upper) || svgRoot.getElementById(lower);
    if (!target) target = svgRoot.querySelector(`g[id='${upper}'], g[id='${lower}']`);
    if (target) target.classList.add("country-seen");
  });
}
